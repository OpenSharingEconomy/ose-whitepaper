...
\subsection{Challenges with Blockchains}
...
\subsection{Fiat income guarantee}

\subsubsection{Fiat Conversion}
...
\subsubsection{Hedging Nodes and Contracts (with Oracle)}

Hedging nodes / contracts
\begin{itemize}
\item Some participant can help the network by running such a node.
\item It automatically <<how?>> accepts future contracts (cf Ethererum Whitepaper) in order to provide insurrance to the user that he'll get the same amount of value in the end. Hedge nodes are rewarded for accepting contracts.
...
A referer associated to each account with KYC.
Default, or no KYC = OSE Foundation.
